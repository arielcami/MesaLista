...
public interface PedidoRepository extends JpaRepository<PedidoEntity, Long> {
	...
	@Query("SELECT p FROM PedidoEntity p WHERE p.delivery.id = :deliveryId AND p.estadoPedido = :estadoPedido AND p.visible = TRUE AND p.paraLlevar = TRUE")
	List<PedidoEntity> buscarPorDeliveryYEstado(
	...
	@Modifying
	@Transactional
	@Query("UPDATE PedidoEntity p SET p.estadoPedido = :nuevoEstado WHERE p.id = :pedidoId AND p.delivery.id = :deliveryId AND p.paraLlevar = TRUE")
